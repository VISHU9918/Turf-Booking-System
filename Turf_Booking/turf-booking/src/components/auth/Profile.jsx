...
const Profile = () => {
	...
	useEffect(() => {
		const fetchBookings = async () => {
			try {
				const response = await getBookingsByUserEmail(userId, token)
				...
				<div className="card p-5 mt-5" style={{ backgroundColor: "whitesmoke" }}>
					...
					<div className="card-body">
						<div className="col-md-10 mx-auto">
							...
							<h4 className="card-title text-center">Turf Booking History</h4>
									...
									<tbody>
										{bookings.map((booking, index) => (
											<tr key={index}>
												...
												<td>{booking.turf.turfType}</td>
												<td>
													{moment(booking.checkInTime).format("HH:mm A")}
												</td>
												<td>
													{moment(booking.checkOutTime).format("HH:mm A")}
												</td>
