...
public class TurfService implements ITurfService {
    ...
    @Override
    public Turf addNewTurf(MultipartFile file, String turfLocation, BigDecimal turfPrice) throws SQLException, IOException {
        Turf turf = new Turf();
        turf.setTurfLocation(turfLocation);  // Changed from turfType to turfLocation
    ...
    @Override
    public List<String> getAllTurfLocations() {
        return turfRepository.findDistinctTurfLocations();  
    }
    ...
    @Override
    public Turf updateTurf(Long turfId, String turfLocation, BigDecimal turfPrice, byte[] photoBytes) {
        Turf turf = turfRepository.findById(turfId)
                .orElseThrow(() -> new ResourceNotFoundException("Turf not found with ID: " + turfId));

        if (turfLocation != null) turf.setTurfLocation(turfLocation);  
    ...
    @Override
    public List<Turf> getAvailableTurfs(LocalDate checkInDate, LocalDate checkOutDate, String turfLocation) {
        return turfRepository.findAvailableTurfsByDatesAndLocation(checkInDate, checkOutDate, turfLocation);  // Changed from turfType to turfLocation
    }

    @Override
    public byte[] getTurfPhotoByTurfId(Long turfId) throws SQLException {
